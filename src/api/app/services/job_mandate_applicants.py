from typing import List

from pydantic import ValidationError
from fastapi import HTTPException, status as http_status

from app.core.logger import logger

from app.db.postgres import get_db

from app.models.user_login import UserDetails
from app.models.job_mandate_applicants import Status, Model

from app.repositories.job_mandate_applicants import Repository


class Service:
    def __init__(self, repo: Repository):
        self.repo = repo

    def create(self, model: Model) -> Model:
        try:
            table = self.repo.get_by_id(
                job_id=model.job_mandate_id,
                applicant_id=model.applicant_id,
                recruiter_id=model.recruiter_id,
            )
            if table:
                logger.debug("Record already exists")
                raise HTTPException(
                    status_code=http_status.HTTP_409_CONFLICT,
                    detail="Record already exists",
                )
            table = self.repo.create(model)
            if table:
                model = Model.model_validate(table)
                logger.debug(f"Created record: {model}")
                return model
            else:
                raise HTTPException(
                    status_code=http_status.HTTP_500_INTERNAL_SERVER_ERROR,
                    detail="Failed to create record",
                )
        except ValidationError as e:
            logger.error(f"Error creating record: {e}")
            raise HTTPException(
                status_code=http_status.HTTP_400_BAD_REQUEST,
                detail="Invalid data provided",
            )

    def create_many(
        self, user_details: UserDetails, applicant_id: int, job_ids: List[int]
    ) -> List[Model]:
        try:
            models = []
            for rank, job_id in enumerate(job_ids, start=1):
                model = Model(
                    recruiter_id=user_details.id,
                    applicant_id=applicant_id,
                    job_mandate_id=job_id,
                    rank=rank,
                )  # type: ignore
                table = self.create(model)
                models.append(Model.model_validate(table))
            return models
        except ValidationError as e:
            logger.error(f"Error creating records: {e}")
            raise HTTPException(
                status_code=http_status.HTTP_400_BAD_REQUEST,
                detail="Invalid data provided",
            )

    def get(self, id: int) -> Model:
        try:
            table = self.repo.get(id)
            if not table:
                raise HTTPException(
                    status_code=http_status.HTTP_404_NOT_FOUND,
                    detail="Record not found",
                )
            Model.model_validate(table)
            return Model.model_validate(table)
        except ValidationError as e:
            logger.error(f"Error fetching record: {e}")
            raise HTTPException(
                status_code=http_status.HTTP_400_BAD_REQUEST,
                detail="Invalid data provided",
            )

    def get_all(self) -> list[Model]:
        try:
            tables = self.repo.get_all()
            if not tables:
                raise HTTPException(
                    status_code=http_status.HTTP_404_NOT_FOUND,
                    detail="No records found",
                )
            return [Model.model_validate(table) for table in tables]
        except ValidationError as e:
            logger.error(f"Error fetching records: {e}")
            raise HTTPException(
                status_code=http_status.HTTP_400_BAD_REQUEST,
                detail="Invalid data provided",
            )

    def get_by_applicant_id(
        self, user_details: UserDetails, applicant_id: int
    ) -> List[Model]:
        try:
            tables = self.repo.get_by_applicant_id(
                applicant_id=applicant_id,
                recruiter_id=user_details.id,
            )
            if not tables:
                raise HTTPException(
                    status_code=http_status.HTTP_404_NOT_FOUND,
                    detail="No records found",
                )
            return [Model.model_validate(table) for table in tables]
        except ValidationError as e:
            logger.error(f"Error fetching records: {e}")
            raise HTTPException(
                status_code=http_status.HTTP_400_BAD_REQUEST,
                detail="Invalid data provided",
            )

    def get_by_applicant_id_and_status(
        self, applicant_id: int, status: Status
    ) -> Model:
        try:
            # Use existing repository method to get record by applicant_id
            record = self.repo.get_by_applicant_id_and_status_ordered(
                applicant_id, status
            )
            if not record:
                raise HTTPException(
                    status_code=http_status.HTTP_404_NOT_FOUND,
                    detail="Record not found",
                )
            model = Model.model_validate(record)
            return model
        except ValidationError as e:
            logger.error(f"Error fetching record by applicant_id and status: {e}")
            raise HTTPException(
                status_code=http_status.HTTP_409_CONFLICT,
                detail="Invalid data provided",
            )

    def update_status(
        self,
        user_details: UserDetails,
        applicant_id: int,
        job_mandate_id: int,
        status: str,
    ) -> Model:
        try:
            table = self.repo.get_by_id(
                job_id=job_mandate_id,
                applicant_id=applicant_id,
                recruiter_id=user_details.id,
            )
            if not table:
                raise HTTPException(
                    status_code=http_status.HTTP_404_NOT_FOUND,
                    detail="Record not found",
                )
            updated_rows = self.repo.update_status(
                job_mandate_id=job_mandate_id, applicant_id=applicant_id, status=status
            )
            if updated_rows != 1:
                logger.error(f"Unexpected number of rows updated: {updated_rows}")
                raise HTTPException(
                    status_code=http_status.HTTP_500_INTERNAL_SERVER_ERROR,
                    detail="Failed to update record",
                )
            logger.debug(f"Record updated successfully")
            table = self.repo.get_by_id(
                job_id=job_mandate_id,
                applicant_id=applicant_id,
                recruiter_id=user_details.id,
            )
            return Model.model_validate(table)
        except ValidationError as e:
            logger.error(f"Error fetching updated record: {e}")
            raise HTTPException(
                status_code=http_status.HTTP_400_BAD_REQUEST,
                detail="Invalid data provided",
            )

    def delete(self, applicant_id: int, recruiter_id: int) -> None:
        repo = Repository(get_db())
        logger.info(f"[delete] Deleting applicant {applicant_id}")
        repo.delete(recruiter_id=recruiter_id, applicant_id=applicant_id)
        repo.close()
